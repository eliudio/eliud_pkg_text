...
class HtmlCache implements HtmlRepository {
  ...
  static Future<HtmlModel> refreshRelations(HtmlModel model) async {
    ...
    List<HtmlMediumModel>? htmlMediaHolder;
    if (model.htmlMedia != null) {
      htmlMediaHolder = List<HtmlMediumModel>.from(await Future.wait(await model.htmlMedia!.map((element) async {
        return await HtmlMediumCache.refreshRelations(element);
      }))).toList();
    }
